...
function CardModal() {
    ...
    const handleDownload = async () => {
        try {
            ...
            link.href = url;
            link.download = selectedCard.filename || "wallpaper.jpg";
...
function CardGalaxy() {
    ...
    return (
        <>
            ...
            {cards.map((card, i) => (
                <FloatingCard key={card.id} card={card} position={cardPositions[i] || { x: 0, y: 0, z: 0, rotationX: 0, rotationY: 0, rotationZ: 0 }} />
            ))}
        </>
    )
}
...
export default function StellarCardGallery({ wallpapers, onClose }: { wallpapers: WallpaperItem[], onClose: () => void }) {
    const cards = useMemo(() => wallpapers.map((w, i) => ({
        id: i.toString(),
        imageUrl: w.src,
        alt: w.alt,
        title: w.alt.length > 20 ? w.alt.substring(0, 20) + "..." : w.alt,
        filename: w.filename
    })), [wallpapers]);
